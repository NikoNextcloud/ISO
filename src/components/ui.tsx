...
export function Section({
  ...
  return (
    <section className="scroll-mt-40 py-5" id={id}>
...
export function StatCard({
  ...
  return (
    <div className="min-h-[116px] rounded-lg border border-line bg-white p-5 shadow-soft">
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`inline-flex rounded-md border px-2 py-1 text-xs font-semibold ${statusClass(knownStatus)}`}>{statusLabel(knownStatus)}</span>;
}
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {standards.map((standard) => (
        <span className="rounded-md border border-line bg-panel px-2 py-1 text-xs font-semibold text-slate-700" key={standard}>
          {standard}
        </span>
      ))}
    </div>
