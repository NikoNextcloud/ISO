...
function DocField({ label, example, children }: { label: string; example?: string; children: React.ReactElement<{ className?: string }> }) { return <div className="grid gap-1.5"><label className="grid gap-1.5 text-sm font-medium text-ink">{label}{cloneElement(children, { className: `focus-ring h-10 w-full rounded border border-line bg-white px-3 text-sm font-normal outline-none ${children.props.className ?? ""}` })}</label><CopyExample text={example} /></div>; }
