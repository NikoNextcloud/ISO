...
import {
  ...
  iso90011400145001ExportConfig,
  isoExportReportHeaders,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createIsoSystemArchive(
      await request.json() as IsoExportRequest,
      iso90011400145001ExportConfig,
      await loadActiveTemplatePackage(request, iso90011400145001ExportConfig.code)
    );
    return new Response(new Uint8Array(result.archive), {
      headers: {
        ...
        "X-Document-Count": String(result.documentCount),
        ...isoExportReportHeaders(result.report)
      }
    });
  } catch (error) {
